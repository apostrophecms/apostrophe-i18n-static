...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    const i18nContents = [];
    let insertMissingActive = false;
    const insertMissingQueue = [];
    ...
    self.i18n = function(req, operation, defaultKey) {
      ...
      function insertMissing() {
        insertMissingQueue.push({
          req,
          key,
          defaultValue
        });
        if (!insertMissingActive) {
          insertMissingNow();
        }
      }

      async function insertMissingNow() {
        insertMissingActive = true;
        try {
          while (insertMissingQueue.length) {
            const {
              req,
              key,
              defaultValue
            } = insertMissingQueue.shift();
